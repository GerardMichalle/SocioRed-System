...
//CAMBIAR TITULO AL CAMBIAR DE VENTANA
...
function updateTopbarDate() {
  ...
  document.getElementById('topbarDate').textContent = now.toLocaleDateString('es-PE', opts);
}
...
function showPage(name) {
  ...
  const titles = { dashboard:'Dashboard', clientes:'Gestión de Clientes', pagos:'Registro de Pagos', planes:'Planes de Internet', antenas:'Antenas - Alto Trujillo', reportes:'Reportes', configuracion:'Configuración', detalle:'Detalle del Cliente' };
  document.getElementById('pageTitle').textContent = titles[name] || name;
  currentPage = name;
  // Render pages
  if (name==='dashboard')     updateDashboard();
  if (name==='clientes')      { clientesPaginaActual=1; renderClientes(); }
...
function updateDashboard() {
  ...
  const mesPagos = pagos.filter(p=>p.estado==='Pagado' && p.mes==='Mayo 2026');
  ...
  document.getElementById('stat-ingresos').textContent = 'S/ ' + ingresos.toLocaleString('es-PE',{minimumFractionDigits:2,maximumFractionDigits:2});
  ...
  document.getElementById('leg-aldia').textContent  = `${activos} (${Math.round(activos/total*100)}%)`;
  document.getElementById('leg-deuda').textContent  = `${deudores} (${Math.round(deudores/total*100)}%)`;

  // Bar chart sectores
  const sectores = ['Sector 1','Sector 2','Sector 3','Sector 4','Sector 5'];
  const barWrap = document.getElementById('barChart');
  const maxCount = Math.max(...sectores.map(s => clientes.filter(c=>c.sector===s).length), 1);
  barWrap.innerHTML = sectores.map(s => {
...
function renderColaQueue() {
  ...
  if (!queue.length) { colaList.innerHTML = '<div class="empty-msg">✅ Sin pagos pendientes</div>'; return; }
  colaList.innerHTML = queue.map((item,i) => `
...
function renderClientes() {
  ...
  if (/^\d+$/.test(query) && query) {
    ...
    if (query) clientes = clientes.filter(c =>
  ...
  if (!paged.length) { tbody.innerHTML = '<tr><td colspan="7" class="empty-msg">No se encontraron clientes</td></tr>'; }
  else {
    tbody.innerHTML = paged.map(c => {
      const plan   = getPlan(c.planId);
      const antena = getAntena(c.antenaId);
      const badge  = c.estado==='Al día'
  ...
  document.getElementById('totalClientes').textContent = `Total: ${total} clientes`;
...
function renderPaginacion(total) {
  ...
  for (let i=1; i<=total; i++) {
    ...
    btn.textContent = i;
    btn.onclick = () => { clientesPaginaActual=i; renderClientes(); };
    pg.appendChild(btn);
  }
}
...
function verCliente(id) {
  ...
  document.getElementById('det-plan').textContent   = plan ? plan.nombre+' · '+plan.velocidad+'Mbps · S/'+plan.precio : c.planId;
  document.getElementById('det-fecha').textContent  = c.fechaInstalacion||'—';
  document.getElementById('det-antena').textContent = antena ? antena.nombre : c.antenaId;
  ...
  document.getElementById('det-pila-size').textContent = `Pila: ${stack?.size||0} pagos`;
  const tbody = document.getElementById('det-pagos');
  if (!historial.length) { tbody.innerHTML = '<tr><td colspan="5" class="empty-msg">Sin historial de pagos</td></tr>'; }
  else {
    tbody.innerHTML = historial.map(p => {
      const est = p.estado==='Pagado'
...
function abrirModalCliente(id=null) {
  ...
  planSel.innerHTML = getPlanes().filter(p=>p.estado==='Activo').map(p=>`<option value="${p.id}">${p.nombre} · ${p.velocidad}Mbps · S/${p.precio}</option>`).join('');
  const antSel = document.getElementById('mc-antena');
  antSel.innerHTML = getAntenas().map(a=>`<option value="${a.id}">${a.nombre}</option>`).join('');
  if (id) {
    const c = getCliente(id);
...
function guardarCliente() {
  ...
  if (dni && !/^\d{8}$/.test(dni)) { showToast('DNI debe tener 8 dígitos','error'); return; }
...
function eliminarCliente(id) {
  if (!confirm(`¿Eliminar cliente ${id}? Se borrarán también sus pagos.`)) return;
...
function renderPagos() {
  const query  = (document.getElementById('searchPago')?.value||'').toLowerCase();
  const estado = document.getElementById('filtroPagoEstado')?.value||'';
  let pagos    = getPagos();
  const clientes = getClientes();
  if (query)  pagos = pagos.filter(p=>{
    const c = clientes.find(x=>x.id===p.clienteId);
    return (c?.nombre||'').toLowerCase().includes(query) || p.mes.toLowerCase().includes(query);
  });
  ...
  document.getElementById('totalPagos').textContent = `Total: ${pagos.length} registros`;
}
...
function abrirRegistrarPago(clienteId=null) {
  ...
  mpSel.innerHTML = clientes.map(c=>`<option value="${c.id}">${c.nombre} (ID: ${c.id})</option>`).join('');
...
function onClientePagoChange() {
  ...
  const plan = getPlan(c?.planId);
  document.getElementById('mp-plan').value  = plan ? `${plan.nombre} · S/${plan.precio}` : '—';
...
function renderPlanes() {
  ...
  const clientes = getClientes();
  document.getElementById('tbodyPlanes').innerHTML = planes.map(p => {
...
function abrirModalPlan(id=null) {
  document.getElementById('modalPlanTitulo').textContent = id ? 'Editar Plan' : 'Nuevo Plan';
...
function guardarPlan() {
  ...
  if (!nombre || !vel || !precio) { showToast('Completa todos los campos','error'); return; }
  ...
  if (id) {
    ...
    planes.push({ id: nextId(planes,'P'), nombre, velocidad:vel, precio, estado:'Activo' });
  }
  setPlanes(planes);
...
function eliminarPlan(id) {
  ...
  if (clientes.length) { showToast(`No puedes eliminar: ${clientes.length} clientes usan este plan`,'error'); return; }
...
function renderAntenas() {
  const antenas  = getAntenas();
  const clientes = getClientes();
  document.getElementById('antenasGrid').innerHTML = antenas.map(a => {
...
function abrirModalAntena(id=null) {
  document.getElementById('modalAntenaTitulo').textContent = id ? 'Editar Antena' : 'Nueva Antena';
...
function editarAntena(id) { abrirModalAntena(id); }
...
function eliminarAntena(id) {
  ...
  if (clientes.length) { showToast(`No puedes eliminar: ${clientes.length} clientes usan esta antena`,'error'); return; }
...
function renderReportes() {
  ...
  const mesPagados   = pagos.filter(p=>p.mes==='Mayo 2026'&&p.estado==='Pagado');
  const mesPendientes= pagos.filter(p=>p.mes==='Mayo 2026'&&p.estado==='Pendiente');
  const totalMes     = mesPagados.reduce((s,p)=>s+p.monto,0);
  const efectividad  = clientes.length ? Math.round(clientes.filter(c=>c.estado==='Al día').length/clientes.length*100) : 0;

  document.getElementById('rep-totalMes').textContent   = `S/ ${totalMes.toLocaleString('es-PE',{minimumFractionDigits:2})}`;
  ...
  // Chart planes
  const planes   = getPlanes();
  const maxC     = Math.max(...planes.map(p=>clientes.filter(c=>c.planId===p.id).length),1);
  document.getElementById('planChart').innerHTML = planes.map(p=>{
  ...
  const meses = ['Diciembre 2025','Enero 2026','Febrero 2026','Marzo 2026','Abril 2026','Mayo 2026'];
  const maxM  = Math.max(...meses.map(m=>pagos.filter(p=>p.mes===m&&p.estado==='Pagado').reduce((s,p)=>s+p.monto,0)),1);
  document.getElementById('histChart').innerHTML = meses.map(m=>{
    const total = pagos.filter(p=>p.mes===m&&p.estado==='Pagado').reduce((s,p)=>s+p.monto,0);
...
function renderBSTVisual() {
  document.getElementById('bst-visual').textContent = clientesBST.toAscii();
}


function renderConfiguracion() {
  document.getElementById('bst-node-count').textContent = `Nodos: ${clientesBST.size}`;
  document.getElementById('queue-size').textContent     = `Elementos: ${pagosQueue.size}`;
  // Contar ops totales de stacks
  let total = 0;
  Object.values(pagosStack).forEach(s => { if(s) total += s.size; });
